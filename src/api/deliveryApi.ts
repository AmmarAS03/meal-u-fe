...
> => {
  ...
  const fetchDeliveryLocations = async (): Promise<DeliveryLocation[]> => {
    const response = await fetch(
      "http://meal-u-api.nafisazizi.com:8001/api/v1/orders/delivery-locations/",
      {
        headers: {
...
export const useDeliveryTimeSlots = (): UseQueryResult<
  DeliveryTimeSlot[],
  Error
> => {
  const { getToken } = useAuth();
  ...
  const fetchDeliveryTimeSlots = async (): Promise<DeliveryTimeSlot[]> => {
    const response = await fetch(
      "http://meal-u-api.nafisazizi.com:8001/api/v1/orders/delivery-time-slots/",
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export const useCreateOrder = () => {
  ...
  return useMutation<OrderCreationResponse, Error, CreateOrderPayload>({
    mutationFn: async (payload) => {
      const token = getToken() || "";
      const response = await fetch(
        "http://meal-u-api.nafisazizi.com:8001/api/v1/orders/checkout/",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
